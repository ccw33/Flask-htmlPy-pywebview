...
def get_netcard():
    ...
    for netcard_name,v in net_info.items():
        if len(v) == 1:
            netcard_info.append({'lan':netcard_name, 'ip':'', 'subnet_mask':'', 'gateway':'', 'dns':'','is_auto':'false'})
        else:
            for item in v:
                if item[0] == 2 and not item[1] == '127.0.0.1':
                    netcard_info.append({'lan':netcard_name, 'ip':item[1], 'subnet_mask':item[2],
                                         'gateway':get_gateway(netcard_name), 'dns':get_dns(),'is_auto':'false'})
    return netcard_info
